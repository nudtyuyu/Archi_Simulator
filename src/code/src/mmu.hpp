...
#pragma once    // ecall和cpu都会include一遍
...
void do_no_page(char *VA, unsigned long error_code);
...
int WriteM(char*VA,const char*buf,int n) //用户态的读内存
{
        int32_t PA = VA2PA(VA,0);
        int32_t i,j=0;
        if(buf==NULL)
...
int WriteMem(char*buf,int addr,int n)  //内核态使用的WriteMem
{
        int i;
        int j = 0;
        if(buf==NULL)
...
void bread_page(int page,char**Disk,int* nb) //将磁盘中的内容读到新分配的页中
{
        //FILE*file;
...
void do_no_page(char*VA,unsigned long error_code) //缺页处理
{
...
int32_t VA2PA(char* VA,int op) //虚地址映射到实地址,op表示虚实地址转换涉及的操作（读还是写）
{
        //op = 0 读该地址，op = 1 写该地址
...
void init() {
        MEM::init();
        TBL1 = (FirstPgTBL*)malloc(TABLE_SIZE*(sizeof(DirEntry))+sizeof(int));
